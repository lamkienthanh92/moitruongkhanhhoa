# BREATHKHANHHOA — One Health Exposure Explorer

Bản nâng cấp từ app gốc (COPD/PM2.5 exposure) sang công cụ đánh giá phơi
nhiễm môi trường đa lớp theo khung One Health, hiện đang chạy cho **tỉnh
Khánh Hòa** (tỉnh mới, sáp nhập Khánh Hòa cũ + Ninh Thuận cũ từ 7/2025).

> **Ghi chú chuyển đổi vùng dữ liệu:** toàn bộ phần lịch sử phát triển
> bên dưới (các vòng sửa lỗi, quyết định thiết kế) mô tả quá trình xây
> dựng bản gốc cho TP.HCM — giữ nguyên vì vẫn đúng về mặt lịch sử và giải
> thích *vì sao* code viết như hiện tại. Dữ liệu thực tế (`grids.json`,
> `sources.js`, `newSourcesData.js`, `roads.js`, `air.js`, `ndvi.js`)
> và ranh giới phường (`wards.js`, để rỗng có chủ đích) đã được thay
> hoàn toàn bằng dữ liệu Khánh Hòa, lấy qua Google Earth Engine +
> Overpass API, không còn dữ liệu TP.HCM nào trong repo này.
